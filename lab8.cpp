/* Name: Matthew Gaitan
 * Assignment: Midterm/ lab8
 * Purpose: To implement a test function that tests the bodyShapeIndex
 * 			function and sees if any of the results are outside of a 
 *			given tolerance.
 */
//CMPS 335
//Exam framework.
...
void unitTestSBSI(double tolerance)
{
	int numBadElements=0;
	for (int row = 0; row < NROWS; row++) {
		SBSI sbsi;
		sbsi.h = testData[row][0];
		sbsi.wc = testData[row][1];
		sbsi.bsa = testData[row][3];
		sbsi.vtc = testData[row][2];
		double output = bodyShapeIndex(&sbsi);

		printf("%.0lf  %3.0lf  %5.0lf  %3.0lf  %.7lf  %.7lf",
			testData[row][0], testData[row][1], testData[row][3],
			testData[row][2], testData[row][4], output);
		
		if (fabs(output - testData[row][4]) > tolerance) {
			cout <<  " <---- outside of tolerance";
			numBadElements++;
		}
		cout << endl;
	}
	cout << "\nbad element count: " << numBadElements << endl;
}
